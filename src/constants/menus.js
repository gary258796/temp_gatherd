export const MENUS = [
  {
    id: 0,
    cn: {
      location: "台北市文山區",
      address: "台北市文山區汀州路四段 191 號 13 樓",
      addressGuide: "萬隆站4號出口，走羅斯福路五段212巷，步行450公尺",
      name: "泰北料理來台灣",
      description:
        "我知道很多台灣人很喜歡吃泰國料理，但是我覺得很難找到道地而且美味的泰國料理。很多你們在台灣吃的「泰式料理」其實在泰國吃不到，因為那不是真的泰國料理！\n\n泰式料理沒有椒麻雞，也沒有月亮蝦餅，打拋飯也不會加蕃茄，【打拋】這個名字其實是從葉子的名字來的，我們使用打拋葉炒而不是九層塔。\n\n泰國北部、東北部、中部、南部的料理味道及使用的材料都不一樣，希望你們可以一起來到這個體驗，讓你們嚐嚐道地而且沒有吃過的泰國料理！",
      detail:
        "體驗中大致上會有 3-6 道菜。\n\n以松阪豬、涼拌玉米鹹蛋、椰奶雞湯、蒸辣魚肉、酸辣湯和炒青菜為主，但是會因為有些食材在台灣很難買得到，可能會有驚喜或是創意料理出現，敬請期待。",
      hostIntroduction:
        "我是盈盈，八年前從泰國清邁來台灣讀大學，從此喜歡上了台灣的生活！\n\n我喜歡去國外旅行，吃當地的料理及了解多元的文化。希望也可以把泰國的美食以及文化帶給你們～",
      notice: "",
      duration: 2,
      price: 500,
      persons: "2-6",
      images: [
        "https://drive.google.com/uc?export=view&id=18ucCIflpViOWKUvK-PmouOIIxWDGexWI",
        "https://drive.google.com/uc?export=view&id=13oWbUD_6PTncoEnYPc0M-vN3QoZL9z_k",
        "https://drive.google.com/uc?export=view&id=16rZfMxKKWYULI3iZno9tfYRXr5wYR3iL",
        "https://drive.google.com/uc?export=view&id=1of1WjBkH0MOoFp0HIvpVT6yL9mJ9u0AQ",
        "https://drive.google.com/uc?export=view&id=17f19tm5jpVX0biR4hEGNPzUUA4rEt0iI",
      ],
      allergy: "奶類, 蛋, 魚類",
      hostName: "李盈盈",
      hostImage:
        "https://drive.google.com/uc?export=view&id=1F7MNPI8tpGpHe2MvUpB29ymVft-UgB6c",
      availableTimes: [
        { date: "5月6日 週六", time: "19:00-22:00", id: "2023/05/06" },
        // { date: "4月10日 週一", time: "下午12:00-下午14:00", id: "2023/04/10" },
        // { date: "4月11日 週二", time: "下午12:00-下午14:00", id: "2023/04/11" },
      ],
    },
    en: {
      location: "Wenshan Dist, Taipei City",
      address:
        "13F, No. 191, Section 4, Tingzhou Rd, Wenshan District, Taipei City, Taiwan 116",
      addressGuide:
        "Wanglong Station exit 4 (Green line), turn left on Lane 192, Section 5, Luosifu Rd, and walk about 450 m. ",
      name: "Northern Thai Dinner in Taiwan",
      description:
        'I understand that many Taiwanese people enjoy Thai cuisine, but I find it difficult to have authentic and delicious Thai food. Many of the "Thai-style dishes" here in Taiwan are actually not available in Thailand because they are not original Thai cuisine.\n\nThai cuisine does not include dishes like "Shredded stir fried pork" or "Moon shrimp cake." Additionally, Thai fried rice does not typically include tomatoes, and the name "Pad Thai" actually comes from the name of the leaf we use for cooking, not basil.\n\nThe flavors and ingredients used in Thai cuisine vary across different regions of Thailand, including the northern, northeastern, central, and southern parts. I invite you to come and experience the cuisine in firsthand, and taste authentic Thai dishes that you haven\'t tried before!\n\nThe dinner will take place in my friend’s residential apartment, where you can over look the Xindian River.',
      detail:
        "In the experience, there will be 3-6 dishes.\n\nInclude:\nMatsusaka pork\nsalted egg corn salad\ncoconut milk chicken soup\nspicy steamed fish\nhot and sour soup\nstir-fried greens\n\nDue to the difficulty of finding certain ingredients in Taiwan, there might be surprises or creative dishes that emerge!",
      hostIntroduction:
        "Hi I’m Ying Ying. Eight years ago, I came from Chiang Mai, Thailand to Taiwan to attend university, and since then, I have fallen in love with the Taiwanese way of life!\n\nI enjoy traveling abroad, tasting local cuisine, and learning about diverse cultures. I hope to share the beauty of Thai food and culture with all of you.",
      notice: "",
      duration: 2,
      price: 500,
      persons: "2-6",
      images: [
        "https://drive.google.com/uc?export=view&id=18ucCIflpViOWKUvK-PmouOIIxWDGexWI",
        "https://drive.google.com/uc?export=view&id=13oWbUD_6PTncoEnYPc0M-vN3QoZL9z_k",
        "https://drive.google.com/uc?export=view&id=16rZfMxKKWYULI3iZno9tfYRXr5wYR3iL",
        "https://drive.google.com/uc?export=view&id=1of1WjBkH0MOoFp0HIvpVT6yL9mJ9u0AQ",
        "https://drive.google.com/uc?export=view&id=17f19tm5jpVX0biR4hEGNPzUUA4rEt0iI",
      ],
      allergy: "奶類, 蛋, 魚類",
      hostName: "李盈盈",
      hostImage:
        "https://drive.google.com/uc?export=view&id=1F7MNPI8tpGpHe2MvUpB29ymVft-UgB6c",
      availableTimes: [
        { date: "5月6日 週六", time: "19:00-22:00", id: "2023/05/06" },
        // { date: "4月10日 週一", time: "下午12:00-下午14:00", id: "2023/04/10" },
        // { date: "4月11日 週二", time: "下午12:00-下午14:00", id: "2023/04/11" },
      ],
    },
  },
  {
    id: 1,
    cn: {
      location: "台北市文山區",
      address: "台北市文山區汀州路四段 191 號 13 樓",
      addressGuide: "萬隆站4號出口，走羅斯福路五段212巷，步行450公尺",
      name: "西班牙不思議",
      description:
        "我們透過西班牙最經典的料理來認識他！從開胃菜到前菜到主食加上最後的甜點來做個 Happy Ending。\n\n除了美食以外會想讓大家看見每道料理背後的故事、文化和西班牙人對生活的那種熱情、享受和態度。",
      detail:
        "前菜和開胃菜屬於西班牙 Tapas 類型的食物 （馬鈴薯烘蛋、蒜香蝦...）。主食我們會由西班牙燉飯或是燉鍋類的食物來當主角。最後大家會吃到像是巴斯克起司蛋糕、米布丁、奶酪、提拉米蘇等甜點！\n\n另有提供加購～\n西班牙頂級黑標 100% 橡子伊比利火腿（如圖所示）\n加購價：1300$/50g，約2-3人份，一人建議15-20g\n如需加購請於訂單備註留言「加購火腿x份」。",
      hostIntroduction:
        "大家好！我是Ricky 一位西班牙土生土長的台灣人。碩士畢業後決定踏上我熱愛的廚藝之路去探索人、美食和文化的連結。\n\n我認為美食是一個接觸文化最佳的起點，希望透過廚藝來傳達美食和文化的連結。並藉由自己對於廚藝和美食的熱忱、愛好和經驗分享西班牙料理和文化！",
      notice: "",
      duration: 2,
      price: 1000,
      persons: "4-8",
      images: [
        "https://drive.google.com/uc?export=view&id=1PdfdCgy9HRrGZBsZcqzUDfl6S5SHJRyw",
        "https://drive.google.com/uc?export=view&id=1VFlcbD74OK_k8k5NwFK64akxRBy01tB_",
        "https://drive.google.com/uc?export=view&id=1iEIK4uYSTh_TNFOBmyAT_hyMXRxq82S_",
        "https://drive.google.com/uc?export=view&id=1mDpuorZbqeFD_t_77st3l_1u1ZTFwvgH",
        "https://drive.google.com/uc?export=view&id=13Jm5Fi5RLfr7qG2Hj0msyLAMDavKqUID",
      ],
      allergy: "奶類, 蛋, 貝類",
      hostName: "Ricardo Yang 楊尚儒",
      hostImage:
        "https://drive.google.com/uc?export=view&id=1LnoHdwZ6cBBFm_VHEHrki2JOFaVp2MN0",
      availableTimes: [
        { date: "5月30日 週三", time: "19:00-21:00", id: "2023/05/30" },
        { date: "6月1日 週四", time: "19:00-21:00", id: "2023/06/01" },
        { date: "6月2日 週五", time: "19:00-21:00", id: "2023/06/02" },
        { date: "6月4日 週日", time: "19:00-21:00", id: "2023/06/04" },
        { date: "6月5日 週一", time: "19:00-21:00", id: "2023/06/05" },
        { date: "6月6日 週二", time: "19:00-21:00", id: "2023/06/06" },
        { date: "6月7日 週三", time: "19:00-21:00", id: "2023/06/07" },
      ],
    },
    en: {
      location: "Wenshan Dist, Taipei City",
      address:
        "13F, No. 191, Section 4, Tingzhou Rd, Wenshan District, Taipei City, Taiwan 116",
      addressGuide:
        "Wanglong Station exit 4 (Green line), turn left on Lane 192, Section 5, Luosifu Rd, and walk about 450 m. ",
      name: "Dinner with a Spanish chef",
      description:
        "I will introduce the most iconic dishes of Spain! From appetizers to starters, main courses, and finally, a sweet dessert to end on a happy note.\n\nApart from showcasing the delicious cuisine, I would also like to introduce you the stories and cultural meaning behind each dish, as well as the passion, enjoyment, and attitude towards life that Spaniards live.\n\nThe dinner will take place in one of my friend's residential apartment, where you can over look the Xindian River.",
      detail:
        'For the starters and appetizers, we will serve classic Spanish tapas such as potato omelette (tortilla española) and garlic shrimp (gambas al ajillo). As for the main course, we will feature paella or other Spanish rice pot. For the grand finale, you can expect indulgent desserts like Basque cheesecake, rice pudding, cheese platters, tiramisu, and more!\n\nAdditionally, we offer an optional upgrade: Spanish Premium Black Label 100% Acorn-fed Iberian Ham (as shown in the picture) Add-on price: $1300/50g, approximately serving 2-3 people, recommended serving size per person is 15-20g. If you would like to add this to your order, please leave a note in the inquiry section with "Add-on ham x serving".',
      hostIntroduction:
        "Hello everyone! I'm Ricky, a Spanish born and raised Taiwanese. After completing my master's degree, I decided to embark on a culinary journey to explore the connection between people, food, and culture, which I'm deeply passionate about.\n\nI believe that food is the best starting point to delve into a culture, and I aspire to bring the link between food and culture through my culinary skills. With my enthusiasm, love, and experiences in cooking and gastronomy, I look forward to sharing my Spanish cuisine and culture with you all!",
      notice: "",
      duration: 2,
      price: 1000,
      persons: "4-8",
      images: [
        "https://drive.google.com/uc?export=view&id=1PdfdCgy9HRrGZBsZcqzUDfl6S5SHJRyw",
        "https://drive.google.com/uc?export=view&id=1VFlcbD74OK_k8k5NwFK64akxRBy01tB_",
        "https://drive.google.com/uc?export=view&id=1iEIK4uYSTh_TNFOBmyAT_hyMXRxq82S_",
        "https://drive.google.com/uc?export=view&id=1mDpuorZbqeFD_t_77st3l_1u1ZTFwvgH",
        "https://drive.google.com/uc?export=view&id=13Jm5Fi5RLfr7qG2Hj0msyLAMDavKqUID",
      ],
      allergy: "奶類, 蛋, 貝類",
      hostName: "Ricardo Yang 楊尚儒",
      hostImage:
        "https://drive.google.com/uc?export=view&id=1LnoHdwZ6cBBFm_VHEHrki2JOFaVp2MN0",
      availableTimes: [
        { date: "5月30日 週三", time: "19:00-21:00", id: "2023/05/30" },
        { date: "6月1日 週四", time: "19:00-21:00", id: "2023/06/01" },
        { date: "6月2日 週五", time: "19:00-21:00", id: "2023/06/02" },
        { date: "6月4日 週日", time: "19:00-21:00", id: "2023/06/04" },
        { date: "6月5日 週一", time: "19:00-21:00", id: "2023/06/05" },
        { date: "6月6日 週二", time: "19:00-21:00", id: "2023/06/06" },
        { date: "6月7日 週三", time: "19:00-21:00", id: "2023/06/07" },
      ],
    },
  },
  {
    id: 2,
    cn: {
      location: "台北市文山區",
      address: "台北市文山區汀州路四段 191 號 13 樓",
      addressGuide: "萬隆站4號出口，走羅斯福路五段212巷，步行450公尺",
      name: "家常特色料理",
      description:
        "好好吃飯、好好生活\n是我在忙碌的生活中休息的方式\n慢慢為自己煮一頓可口簡單的料理\n\n每一道菜都有不同的回憶\n\n家族傳下來的、和朋友一起旅行發明的、外國朋友教的、別人的家鄉菜……\n\n菜色就像一張張相片\n打開記憶的味蕾\n那些飽滿美麗的回憶彷彿又重溫一遍",
      detail:
        "4道家常特色料理，主食為白飯和1-2份手工歐式麵包\n\n佐醬口味皆為辛辣",
      hostIntroduction: "喜歡在空閒的時候\n\n慢慢料理一頓美味的餐點",
      notice: "",
      duration: 2.5,
      price: 480,
      persons: "3-7",
      images: [
        "https://drive.google.com/uc?export=view&id=1Zswt1yRg4cHPR7mU5wymc0p4nDsqqKge",
        "https://drive.google.com/uc?export=view&id=1Lfqd7-oIw2tEGbce81NBGClWffxPpGwE",
        "https://drive.google.com/uc?export=view&id=1XQJsJ8s0-U_XuXaTOOjjaM34jlS5B6Bg",
        "https://drive.google.com/uc?export=view&id=1iPbVHixEMQO7lyJnmOVdiqkhAVTAmMMI",
        "https://drive.google.com/uc?export=view&id=1UTbaIbUt_bx1BfCkqT6gSNVDHLigBNeR",
      ],
      allergy: "辛辣、蝦子（過敏）",
      hostName: "Helen",
      hostImage:
        "https://drive.google.com/uc?export=view&id=13QEyO2uEb5UYQiguTMJKvtbK4pb4k-5C",
      availableTimes: [],
    },
    en: {
      location: "Wenshan Dist, Taipei City",
      address:
        "13F, No. 191, Section 4, Tingzhou Rd, Wenshan District, Taipei City, Taiwan 116",
      addressGuide:
        "Wanglong Station exit 4 (Green line), turn left on Lane 192, Section 5, Luosifu Rd, and walk about 450 m. ",
      name: "Homemade Taiwanese cuisine with Helen",
      description:
        "Enjoy a meal, and live well. It's my way of relaxation in a busy life. More important, to cook a delicious and simple meal for myself.\n\nEach dish carries different memories. Passed down by my family, discover while traveling with friends, taught by foreign friends, or dishes from friends hometowns\n\nThe dishes are like photographs. Opening up the taste buds of memories. Those fulfilling and beautiful memories seem to be relived once again.",
      detail:
        "4 home-style specialty dishes, with steamed rice and 1-2 servings of homemade bread.\n\nPlease note that all dishes are a bit spicy.",
      hostIntroduction:
        "Hi I’m Helen, I enjoy discover hidden food places, and cooking leisurely during my free time.",
      notice: "",
      duration: 2.5,
      price: 480,
      persons: "3-7",
      images: [
        "https://drive.google.com/uc?export=view&id=1Zswt1yRg4cHPR7mU5wymc0p4nDsqqKge",
        "https://drive.google.com/uc?export=view&id=1Lfqd7-oIw2tEGbce81NBGClWffxPpGwE",
        "https://drive.google.com/uc?export=view&id=1XQJsJ8s0-U_XuXaTOOjjaM34jlS5B6Bg",
        "https://drive.google.com/uc?export=view&id=1iPbVHixEMQO7lyJnmOVdiqkhAVTAmMMI",
        "https://drive.google.com/uc?export=view&id=1UTbaIbUt_bx1BfCkqT6gSNVDHLigBNeR",
      ],
      allergy: "辛辣、蝦子（過敏）",
      hostName: "Helen",
      hostImage:
        "https://drive.google.com/uc?export=view&id=13QEyO2uEb5UYQiguTMJKvtbK4pb4k-5C",
      availableTimes: [],
    },
  },
  {
    id: 3,
    cn: {
      location: "台北市文山區",
      address: "台北市文山區汀州路四段 191 號 13 樓",
      addressGuide: "萬隆站4號出口，走羅斯福路五段212巷，步行450公尺",
      name: "與Candy的特色料理",
      description:
        "結合中式傳統烹技巧和在英國生活期間發現的新穎創新食材一併融合到料理裡。優先使用當地市場採購的季節性食材。我的烹飪熱情源於外婆，並不斷尋找不同元素的食材融入到料理中。我認為食物是用來分享和享受的。與我一起體驗難忘的用餐時光，是慶祝多元烹飪的最佳結合。",
      detail:
        "品嚐由市場新鮮食材精心烹調的五到六道無國界料理，也有機會品嚐到意外驚喜的額外料理。",
      hostIntroduction:
        "Hi 我是Candy，充滿活力與熱情。具有外向的個性和分享的熱愛，總是渴望嘗試新事物並探索周圍的世界。",
      notice: "",
      duration: 3,
      price: 650,
      persons: "1-4",
      images: [
        "https://drive.google.com/uc?export=view&id=1BmQ8CmmTiwI9UUQ7ibeoHa40NpNWCY8g",
        "https://drive.google.com/uc?export=view&id=1AzJilP1ppZTzaGR1c3d5UOvJb7htq-AY",
        "https://drive.google.com/uc?export=view&id=1GxetjoPR4OzWieTsA3HJjrJUJvAIXKo-",
        "https://drive.google.com/uc?export=view&id=1PtuwVOvxIgxevCdNBGxmm3Pfz2OsDPtm",
        "https://drive.google.com/uc?export=view&id=133qbehE6bLAeUjxHsnvk1umC-AhIBA12",
      ],
      allergy: "奶類, 蛋, 貝類, 魚類",
      hostName: "Candy",
      hostImage:
        "https://drive.google.com/uc?export=view&id=1M-6jvc1kkyO5-KOid4beksO6KLJkErA7",
      availableTimes: [
        { date: "5月28日 週日", time: "18:00-21:00", id: "2023/05/28" },
        { date: "6月3日 週六", time: "18:00-21:00", id: "2023/06/03" },
        { date: "6月10日 週六", time: "18:00-21:00", id: "2023/06/10" },
        { date: "6月17日 週六", time: "18:00-21:00", id: "2023/06/17" },
      ],
    },
    en: {
      location: "Wenshan Dist, Taipei City",
      address:
        "13F, No. 191, Section 4, Tingzhou Rd, Wenshan District, Taipei City, Taiwan 116",
      addressGuide:
        "Wanglong Station exit 4 (Green line), turn left on Lane 192, Section 5, Luosifu Rd, and walk about 450 m. ",
      name: "Candy’s fusion cuisine",
      description:
        "The experience will combining traditional Chinese culinary techniques with innovative ingredients where I discovered during my time in the UK.\n\nI prioritize using seasonal ingredients sourced from local markets. My passion for cooking stems from my grandmother, and I continuously seek to incorporate different elements and ingredients into my cuisine. I believe that food is meant to be shared and enjoyed. Join me a unforgettable dining experience, celebrating the best of diverse culinary dishes.",
      detail:
        "Enjoy 5 to 6 fusion dishes. Prepared using fresh ingredients from the local market. There is also a chance of additional unexpected surprise dishes.",
      hostIntroduction:
        "Hi, I'm Candy, I’m enthusiastic and full of energy. With an outgoing personality and a love for sharing, I'm always eager to try new things and explore the world around me!",
      notice: "",
      duration: 3,
      price: 650,
      persons: "1-4",
      images: [
        "https://drive.google.com/uc?export=view&id=1BmQ8CmmTiwI9UUQ7ibeoHa40NpNWCY8g",
        "https://drive.google.com/uc?export=view&id=1AzJilP1ppZTzaGR1c3d5UOvJb7htq-AY",
        "https://drive.google.com/uc?export=view&id=1GxetjoPR4OzWieTsA3HJjrJUJvAIXKo-",
        "https://drive.google.com/uc?export=view&id=1PtuwVOvxIgxevCdNBGxmm3Pfz2OsDPtm",
        "https://drive.google.com/uc?export=view&id=133qbehE6bLAeUjxHsnvk1umC-AhIBA12",
      ],
      allergy: "奶類, 蛋, 貝類, 魚類",
      hostName: "Candy",
      hostImage:
        "https://drive.google.com/uc?export=view&id=1M-6jvc1kkyO5-KOid4beksO6KLJkErA7",
      availableTimes: [
        { date: "5月28日 週日", time: "18:00-21:00", id: "2023/05/28" },
        { date: "6月3日 週六", time: "18:00-21:00", id: "2023/06/03" },
        { date: "6/10 SAT", time: "18:00-21:00", id: "2023/06/10" },
        { date: "6/17 SAT", time: "18:00-21:00", id: "2023/06/17" },
      ],
    },
  },
  {
    id: 4,
    cn: {
      location: "台北市文山區",
      address: "台北市文山區汀州路四段 191 號 13 樓",
      addressGuide: "萬隆站4號出口，走羅斯福路五段212巷，步行450公尺",
      name: "旬味割烹",
      description:
        "日本料理的特色是透過食物的原味，傳達春夏秋冬的變化以及季節感，調味相對單純卻富有風味。\n\n因為從小就特別喜歡日本的文化，在我成長的過程中從語言、歷史、飲食等等不同面向慢慢認識了這個國家，而飲食是我最拿手的絕活，希望能把這樣的快樂帶給更多的人，歡迎來一同體驗～",
      detail:
        "在這個體驗當中，我們將以類似於本膳料理的方式進行，並依據季節會稍作變動，絕對使用當季食材！\n\n大致上會有煮物、蒸物、揚物、漬物、味增湯或清湯\n\n另外提供代客處理海鮮食材的服務，價格如下：\n\n魚類\n二斤以下200元\n二斤以上300元\n三斤以上400元\n\n蟹200元\n\n蝦、貝類150元\n\n其他則另行討論\n\n提醒您！海鮮請做好保鮮，如有食安疑慮恕不代為處理，謝謝",
      hostIntroduction:
        "大家好我叫愷原，做餐飲業將近十年。學習過程以及料理職涯中，都是以日式的菜系為主，目前在日本料理的餐廳工作。\n\n平常就很喜歡跟好友分享料理，不喜歡被框架的個性也讓我會在家裡自己嘗試各式各樣的料理，除了依照傳統的做法之外，會融入自己偏好的口味以及創意！",
      notice: "素食者可在備註中告知",
      duration: 3,
      price: 850,
      persons: "4-6",
      images: [
        "https://drive.google.com/uc?export=view&id=1L-fwR4mF7vMIhB9izmMEF9n6QhoTTJOz",
        "https://drive.google.com/uc?export=view&id=1OH0jjmXdPeqGgD-szl4OMTL2mBrNwa1Q",
        "https://drive.google.com/uc?export=view&id=1CsainNjDsIHPgSz0BcTKDjAHI9d1ebcU",
        "https://drive.google.com/uc?export=view&id=1f0vNUqR-ZvebQgAD8IDTSFuXwFpyKWjt",
        "https://drive.google.com/uc?export=view&id=1DF1ERD6SfoIpmLbikebji3DEY3uxxiK_",
      ],
      allergy: "奶類, 蛋, 貝類, 魚類, 麩質",
      hostName: "何愷原",
      hostImage:
        "https://drive.google.com/uc?export=view&id=1Vj-z6mGZXtjjJyZm0Tfg9laR3s6fjMZM",
      availableTimes: [
        { date: "6/18 SUN", time: "19:00-22:00", id: "2023/06/18" },
        { date: "6/22 THU", time: "19:00-22:00", id: "2023/06/22" },
        { date: "6/23 FRI", time: "19:00-22:00", id: "2023/06/23" },
      ],
    },
    en: {
      location: "Wenshan Dist, Taipei City",
      address:
        "13F, No. 191, Section 4, Tingzhou Rd, Wenshan District, Taipei City, Taiwan 116",
      addressGuide:
        "Wanglong Station exit 4 (Green line), turn left on Lane 192, Section 5, Luosifu Rd, and walk about 450 m. ",
      name: "Seasonal Japanese cuisine",
      description:
        "The characteristic of Japanese cuisine is to bring the changes of the four seasons and a sense of seasonality through the natural flavors of the ingredients. The seasonings used are relatively simple yet rich in flavors.\n\nGrowing up, I have a special interest for Japanese culture. I have gradually learned about Japan from different aspects such as language, history, and food. Cooking is my specialty, and I hope to bring this kind of joy to more people. I welcome you to join me this experience!\n\nThe dinner will take place in my friend’s residential apartment where you can over look the Xindian River.",
      detail:
        "In this experience, I will follow a similar approach to traditional Japanese cuisine, with slight variations based on the seasonal ingredients!\n\nThe menu will include simmered dishes, steamed dishes, fried dishes, pickles, miso soup, or clear broth.\n\nAdditionally, I also provide a service where you can bring your own seafood and I will prepare during the experience.\n\nHere are the following prices:\nFish:\n* Under 2kg: $200\n* 2kg or above: $300\n* 3kg or above: $400\nCrab: $200\nShrimp and shellfish: $150\nFor other seafood, we can discuss it separately\n\nPlease note that it is important to ensure the freshness of seafood. If there are any food safety concerns, I will respectfully decline. Thank you.",
      hostIntroduction:
        "Hello, My name is Kai Yuan, and I have been working in the food and beverage industry for nearly ten years. Throughout my learning process and culinary career, I have primarily focused on Japanese cuisine, and I am currently working at a Japanese restaurant.\n\nI enjoy sharing my cooking with friends on a regular basis. I don't have a specific cooking framework, which allows me to experiment with various types of dishes at home. In addition of following traditional methods, I incorporate my own flavors and creativity into my cooking!",
      notice: "素食者可在備註中告知",
      duration: 3,
      price: 850,
      persons: "4-6",
      images: [
        "https://drive.google.com/uc?export=view&id=1L-fwR4mF7vMIhB9izmMEF9n6QhoTTJOz",
        "https://drive.google.com/uc?export=view&id=1OH0jjmXdPeqGgD-szl4OMTL2mBrNwa1Q",
        "https://drive.google.com/uc?export=view&id=1CsainNjDsIHPgSz0BcTKDjAHI9d1ebcU",
        "https://drive.google.com/uc?export=view&id=1f0vNUqR-ZvebQgAD8IDTSFuXwFpyKWjt",
        "https://drive.google.com/uc?export=view&id=1DF1ERD6SfoIpmLbikebji3DEY3uxxiK_",
      ],
      allergy: "奶類, 蛋, 貝類, 魚類, 麩質",
      hostName: "何愷原",
      hostImage:
        "https://drive.google.com/uc?export=view&id=1Vj-z6mGZXtjjJyZm0Tfg9laR3s6fjMZM",
      availableTimes: [
        { date: "6/18 SUN", time: "19:00-22:00", id: "2023/06/18" },
        { date: "6/22 THU", time: "19:00-22:00", id: "2023/06/22" },
        { date: "6/23 FRI", time: "19:00-22:00", id: "2023/06/23" },
      ],
    },
  },
];
